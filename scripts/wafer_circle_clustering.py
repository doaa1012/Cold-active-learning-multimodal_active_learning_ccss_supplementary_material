...
def process_wafer_image_saturation_black_seeds(
    image_path, save_folder="output_clusters", k=18, num_seeds=1, base_seed=42, top_n=5
):
    """
    Clean wafer image, enhance with saturation/contrast, apply KMeans clustering.
    If num_seeds > 1, files are saved with seed numbers.
    If num_seeds == 1, filenames do not include seed numbers.
    """
    os.makedirs(save_folder, exist_ok=True)
    ...
    # Versions
    img_versions = [
        ("Saturation_low", adjust_saturation_contrast(cropped_img, 1.25, 1.3)),
        ("Saturation_medium", adjust_saturation_contrast(cropped_img, 1.5, 1.6)),
        ("Saturation_high", adjust_saturation_contrast(cropped_img, 1.75, 1.9)),
    ]
    ...
    for title, image in img_versions:
        for seed in range(base_seed, base_seed + num_seeds):
            ...
            pixel_values = image.reshape((-1, 3)).astype(np.float32)
            non_background = np.any(pixel_values < 245, axis=1)
            filtered = pixel_values[non_background]

            _, labels, centers = cv2.kmeans(filtered, k, None, criteria, 10, cv2.KMEANS_PP_CENTERS)
            centers = np.uint8(centers)

            labels_full = np.full(pixel_values.shape[0], -1)
            labels_full[non_background] = labels.flatten()
            labels_full = labels_full.reshape(image.shape[:2])

            unique, counts = np.unique(labels, return_counts=True)
            sorted_clusters = sorted(dict(zip(unique, counts)), key=dict(zip(unique, counts)).get, reverse=True)[:top_n]
            ...
            for idx, cluster in enumerate(sorted_clusters):
                ...
                # --- Filenames depend on num_seeds ---
                if num_seeds > 1:
                    cluster_filename = f"{title}_seed_{seed}_cluster_{idx+1}.png"
                else:
                    cluster_filename = f"{title}_cluster_{idx+1}.png"

                cv2.imwrite(os.path.join(save_folder, cluster_filename), cluster_image)

            plt.tight_layout()
            # --- Save figure with/without seed ---
            if num_seeds > 1:
                fig_filename = f"{title}_seed_{seed}_top{top_n}_clusters.png"
                centroid_csv = os.path.join(save_folder, f"{title}_seed_{seed}_centroids.csv")
                centroid_pkl = os.path.join(save_folder, f"{title}_seed_{seed}_centroids.pkl")
            else:
                fig_filename = f"{title}_top{top_n}_clusters.png"
                centroid_csv = os.path.join(save_folder, f"{title}_centroids.csv")
                centroid_pkl = os.path.join(save_folder, f"{title}_centroids.pkl")

            fig.savefig(os.path.join(save_folder, fig_filename))
            plt.close(fig)

            # Save centroids
            pd.DataFrame(centroids, columns=["cx","cy"]).to_csv(centroid_csv, index=False)
            with open(centroid_pkl, "wb") as f:
                pickle.dump(centroids, f)

            print(f"Saved: {title} | Seed {seed if num_seeds>1 else ''} | Centroids: {len(centroids)}")
